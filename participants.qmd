...
for (i in 1:nrow(x)) {
    ...
    twitter <- if (x$twitter[i] != "" & !is.na(x$twitter[i])) sprintf(
        '[{{< fa brands twitter >}}](%s)',
        paste0("https://twitter.com/", x$twitter[i])
        ) else ""

    github <- if (x$github[i] != "" & !is.na(x$github[i])) sprintf(
        '[{{< fa brands github >}}](%s)',
        paste0("https://github.com/", x$github[i])
        ) else ""

    web <- if (x$url[i] != "" & !is.na(x$url[i])) sprintf(
        '[{{< fa solid globe >}}](%s)',
        x$url[i]
        ) else ""

    affil <- if (x$affil[i] != "" & !is.na(x$affil[i])) sprintf(
        "**%s**",
        x$affil[i]
        ) else ""

    pronouns <- if (x$pronouns[i] != "" & !is.na(x$pronouns[i])) sprintf(
        "(pronouns: %s)<br>",
        x$pronouns[i]
        ) else ""

    linkedin <- if (x$linkedin[i] != "" & !is.na(x$linkedin[i])) sprintf(
